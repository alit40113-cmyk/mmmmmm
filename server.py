...
def admin_edit_points(m):
    try:
        ...
        bot.send_message(m.chat.id, "✅ تم تعديل النقاط بنجاح.")
...
def admin_create_code(m):
    try:
        ...
        bot.send_message(m.chat.id, f"✅ كود شحن جديد: `{code}`")
...
def router(c):
    ...
    if c.data == "nav_ins":
        ...
        msg = bot.send_message(cid, "أرسل (ID نقاط):")
        bot.register_next_step_handler(msg, admin_edit_points)

    elif c.data == "adm_gen_code":
        msg = bot.send_message(cid, "أرسل (نقاط استخدامات):")
        ...
        if req:
            ...
            else: bot.send_message(cid, "❌ رصيد غير كافٍ.")
