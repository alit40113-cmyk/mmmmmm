# ==========================================================
# 🚀 مـحـرك تـايـتـان V37 - الـنـسـخـة الـمـلـحـمـيـة (الـمُـصـحـحـة)
# 💎 إعادة كافة الأزرار القديمة + نظام الأيام (5 نقاط)
...
def init_db():
    ...
    # إضافة عمود days للطلبات لضمان عمل نظام الحساب الجديد
...
# --- 🔗 معالج الأزرار الشامل (استرجاع كافة الوظائف القديمة) ---
...
def router(c):
    ...
    # --- ⚙️ لوحة الإدارة (كاملة الأزرار) ---
        ...
        for r in reqs:
            kb = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton(f"✅ قبول (خصم {r['days']*5})", callback_data=f"acc_{r['req_id']}"))
            bot.send_message(cid, f"🔔 طلب من: `{r['user_id']}`\n📄 ملف: `{r['file_name']}`\n📅 المدة المختارة: {r['days']} يوم", reply_markup=kb)

    # --- 🕒 نظام اختيار الأيام (الإضافة المطلوبة) ---
        ...
        conn.commit()
        bot.edit_message_text(f"✅ تم اختيار {days} يوم. بانتظار موافقة الإدارة لخصم {int(days)*5} نقاط.", cid, mid)
        bot.send_message(ADMIN_ID, f"🔔 طلب جديد: المستخدم {uid} يريد {days} يوم.")

    # --- ⚙️ وظائف لوحة الإدارة (Next Step Handlers) ---
    elif c.data == "adm_users":
        msg = bot.send_message(cid, "أرسل (ID المستخدم) ثم (النقاط) مثال: `8504553407 10`:")
        bot.register_next_step_handler(msg, admin_edit_points)

    elif c.data == "adm_gen_code":
        msg = bot.send_message(cid, "أرسل (النقاط) ثم (عدد الاستخدامات) مثال: `50 10`:")
        ...
        msg = bot.send_message(cid, "أرسل الكود هنا:")
        bot.register_next_step_handler(msg, user_redeem_code)

    elif c.data == "nav_ins":
        msg = bot.send_message(cid, "📤 أرسل ملف الأداة (.py):")
        ...
        if req:
            ...
            if u_pts < cost:
                bot.send_message(cid, "❌ رصيد المستخدم غير كافٍ للخصم.")
            else:
                ...
                conn.commit()
                bot.send_message(req['user_id'], f"✅ تم تفعيل مشروعك لـ {req['days']} يوم!\n🔗 الرابط: `{BASE_URL}/run/{lid}`")
                bot.delete_message(cid, mid)
    conn.close()

# --- 🛠️ الوظائف المساعدة (نفس الأسماء والمنطق القديم) ---
def admin_edit_points(m):
    try:
        target, amt = m.text.split()
        conn = get_db(); conn.execute('UPDATE users SET points = points + ? WHERE user_id = ?', (int(amt), target)); conn.commit(); conn.close()
...
def handle_upload(m):
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(types.InlineKeyboardButton("يوم (5)", callback_data=f"set_days_1_{rid}"),
           types.InlineKeyboardButton("3 أيام (15)", callback_data=f"set_days_3_{rid}"),
           types.InlineKeyboardButton("أسبوع (35)", callback_data=f"set_days_7_{rid}"),
           types.InlineKeyboardButton("شهر (150)", callback_data=f"set_days_30_{rid}"))
